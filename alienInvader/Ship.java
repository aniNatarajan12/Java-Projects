...
public class Ship implements MouseListener, MouseMotionListener, KeyListener  {
	...
	public void keyPressed(KeyEvent e){ 
		...
		if(e.getKeyCode()==KeyEvent.VK_R){
			ScoreBoard.addTorpedo();
			ScoreBoard.addScore();
		}
		if(e.getKeyCode()==KeyEvent.VK_F){
			int num = (int) (Math.random()*2000);
			if(ScoreBoard.getScore() <= num){
		...
		if(e.getKeyCode()==KeyEvent.VK_Q){
			if(Coordinator.godMode==false){
				Coordinator.godMode = true;
				ScoreBoard.godMode();
			}
			else if(Coordinator.godMode==true){
				Coordinator.godMode=false;
			}
		}
		if(e.getKeyCode()==KeyEvent.VK_E){
			...
			Torpedo torpedo = new Torpedo(x, y-Torpedo.HEIGHT, -5);
			TorpedoManager.add(torpedo);
			Torpedo torpedo1 = new Torpedo(x-15, y-Torpedo.HEIGHT, -5);
			TorpedoManager.add(torpedo1);
			Torpedo torpedo2 = new Torpedo(x+15, y-Torpedo.HEIGHT, -5);
			TorpedoManager.add(torpedo2);
			firingSound.play();
		}
